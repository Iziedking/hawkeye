...
//  Internal types (not exported to bus) 

interface WatchedWallet {
  address: string;
  label?: string;
  /** Restrict monitoring to one chain. Undefined = watch all. */
  chain?: ChainClass;
}
...
function buildTradeIntent(
  ...
  const tokenAddress =
    swap.type === "buy" ? swap.baseToken.address : swap.quoteToken.address;
  ...
  return {
    ...
    urgency: "NORMAL",
    rawText: `Copy trade from ${wallet.label ?? wallet.address}: ` +
...
async function handleSwapEvent(
  ...
  const pairKey = makeDedupKey(
    wallet.address,
    swap.baseToken.address,
    swap.quoteToken.address
  );
...
function connectWallet(wallet: WatchedWallet): void {
  ...
  const doConnect = (): void => {
    console.log(
      `[CopyTrade] Opening WS for ${wallet.label ?? addr} ` +
        `(attempt ${attempts + 1})`
    );
    ...
    ws.addEventListener("close", (evt) => {
      console.warn(
        `[CopyTrade] WS closed — ${addr} code=${evt.code} ` +
...
export function startCopyTradeAgent(): void {
  console.log("[CopyTrade] Agent started");

  // Dynamic wallet management — Gateway calls these when user configures
  // wallets to watch via the bot.
  bus.on("ADD_WATCHED_WALLET" as any, (wallet: WatchedWallet) => {
    const addr = wallet.address.toLowerCase();
    if (watchedWallets.has(addr)) {
      console.log(`[CopyTrade] ${addr} already watched`);
      return;
    }
    watchedWallets.set(addr, wallet);
    connectWallet(wallet);
    console.log(`[CopyTrade] Wallet added — ${wallet.label ?? addr}`);
  });

  bus.on("REMOVE_WATCHED_WALLET" as any, (address: string) => {
