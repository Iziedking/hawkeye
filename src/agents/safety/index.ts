...
async function resolveChain(
  ...
  if (chainClass === "solana") {
    try {
      ...
      return { chainId: "solana", liquidityUsd: pair?.liquidity?.usd ?? 0, priceUsd: parseFloat(pair?.priceUsd ?? "0") || 0 };
    } catch {
  ...
  try {
    ...
    return { chainId: best.chainId as ChainId, liquidityUsd: best.liquidity?.usd ?? 0, priceUsd: parseFloat(best.priceUsd ?? "0") || 0 };
  } catch {
...
type GoPlusData = {
  ...
  is_open_source?: string; honeypot_with_same_creator?: string;
};
...
async function checkGoPlus(
  ...
  try {
    ...
    if (!data) return { flags: [{ flag: "UNVERIFIED_CONTRACT", source: "goplus" }], ok: false, detail: null };
    ...
    if (data.honeypot_with_same_creator === "1") flags.push({ flag: "KNOWN_RUGGER",        source: "goplus" });
...
async function checkHoneypotIs(
  ...
  try {
    ...
    const flags: FlagWithSource[] = body.isHoneypot === true
      ? [{ flag: "HONEYPOT", source: "honeypot" }]
      : [];
...
async function checkRugCheck(
  ...
  try {
    ...
    if (!resp.ok) return { flags: [{ flag: "UNVERIFIED_CONTRACT", source: "rugcheck" }], ok: false, detail: null };
    ...
    for (const risk of report.risks ?? []) {
      ...
      if (name.includes("honeypot") || name.includes("rugged")) flags.push({ flag: "HONEYPOT",        source: "rugcheck" });
      ...
      if (level === "danger" && name.includes("tax"))           flags.push({ flag: "HIGH_TAX",         source: "rugcheck" });
    }
    ...
    return { flags: [{ flag: "UNVERIFIED_CONTRACT", source: "rugcheck" }], ok: false, detail: null };
  }
}
...
type GoPlusSolanaData = {
  ...
  non_transferable?: string; closeable?: string;
};
...
async function checkGoPlusSolana(
  ...
  for (let attempt = 0; attempt <= GOPLUS_RETRY_DELAYS_MS.length; attempt++) {
    ...
    try {
      ...
      return { flags: [{ flag: "UNVERIFIED_CONTRACT", source: "goplusSolana" }], ok: false, detail: null };
    }
  }
  ...
  try {
    ...
    if (!data) return { flags: [{ flag: "UNVERIFIED_CONTRACT", source: "goplusSolana" }], ok: false, detail: null };
...
type AgeAndHolders = { ageHours: number | null; top3Pct: number | null; holderCount: number | null };
...
async function checkSolscan(address: string): Promise<AgeAndHolders> {
  ...
  try {
    ...
    let holderCount: number | null = null;
    if (holdersResp.ok) {
      const body = await holdersResp.json() as { total?: number; data?: Array<{ amount?: number }> };
...
async function scanToken(
  ...
  if (TESTNET_CHAIN_IDS.has(chainId)) {
    ...
    return {
      intentId, address, chainId: chainId as ChainId,
      score: 100, flags: [],
  ...
  if (chainClass === "evm") {
    ...
    sources.push({ provider: "goplus", ok: goplusSol.ok, detail: { ...goplusSol.detail, _solana: true } });
  ...
  if (ageHolders.ageHours !== null && ageHolders.ageHours < 1) {
    allFlags.push({ flag: "LOW_LIQUIDITY", source: "dexscreener" });
    console.log(`[safety] ${address}: very new token (${ageHolders.ageHours.toFixed(1)}h) — adding LOW_LIQUIDITY flag`);
  }

  sources.push({ provider: "dexscreener", ok: true, detail: { chainId, liquidityUsd, priceUsd } });

  let { score, flags } = computeScore(allFlags, jupiterBonus);
  ...
  if (ageHolders.top3Pct !== null && ageHolders.top3Pct > 80) {
    ...
    console.log(`[safety] ${address}: high holder concentration (${ageHolders.top3Pct.toFixed(0)}%) — score -20`);
  }
