...
export function startSwarmTracer(): () => void {
  ...
  const onQuote = (q: Quote): void => {
    ...
    console.log(
      `[swarm ${delta(q.intentId, tradeT0)}] ├── QUOTE: $${q.priceUsd} liq=${liq} slip=${q.expectedSlippagePct.toFixed(1)}%`,
    );
  };
  ...
  const onStrategy = (d: StrategyDecision): void => {
    ...
    const tag =
      d.decision === "EXECUTE" ? "EXECUTE" : d.decision === "REJECT" ? "REJECT" : "CONFIRM?";
  ...
  const onExecuted = (pos: TradeExecutedPayload): void => {
    ...
    console.log(
      `[swarm ${delta(pos.intentId, tradeT0)}] └── EXECUTED tx=${pos.txHash.slice(0, 14)}...`,
    );
  ...
  const onResearchRes = (res: ResearchResult): void => {
    ...
    console.log(
      `[swarm ${delta(res.requestId, researchT0)}] └── RESEARCH_RESULT: safety=${res.safetyScore ?? "N/A"} price=$${res.priceUsd ?? "?"}`,
    );
